...
class BaseModel():
    def __init__(self, opt, phase_loader, val_loader, metrics, logger, writer, wandb_logger):
    ...
    def train(self):
        ...
        while self.epoch <= self.opt['train']['n_epoch'] and self.iter <= self.opt['train']['n_iter']:
            ...
            if self.epoch % self.opt['train']['val_epoch'] == 0 or self.epoch -1 == 0:
                self.logger.info("\n\n\n------------------------------Validation Start------------------------------")
                if self.val_loader is None:
                    ...
                    for key, value in val_log.items():
                        self.logger.info('{:5s}: {}\t'.format(str(key), value))
                    if 'val/mae' in val_log and val_log['val/mae'].item() < best_val_loss:
                        self.save_best_network(self.epoch, self.iter, self.optimizers[0])
                        best_val_loss = val_log['val/mae'].item()
                        self.logger.info(f'Saving best network, val mae={best_val_loss:.3f}...\n\n')
                        print(f'Saving best network, val mae={best_val_loss:.3f}...\n\n')
    ...
    def load_network(self, network, network_label, strict=True):
        ...
        if self.opt['model']['which_networks'][0]['args']['unet']['finetune_bounds']:
            with torch.no_grad():
                network.denoise_fn.lower[0].weight.copy_(network.denoise_fn.out[0].weight)
                network.denoise_fn.lower[0].bias.copy_(network.denoise_fn.out[0].bias)
                network.denoise_fn.lower[2].weight.copy_(network.denoise_fn.out[2].weight)
                network.denoise_fn.lower[2].bias.copy_(network.denoise_fn.out[2].bias)

                network.denoise_fn.upper[0].weight.copy_(network.denoise_fn.out[0].weight)
                network.denoise_fn.upper[0].bias.copy_(network.denoise_fn.out[0].bias)
                network.denoise_fn.upper[2].weight.copy_(network.denoise_fn.out[2].weight)
                network.denoise_fn.upper[2].bias.copy_(network.denoise_fn.out[2].bias)
    ...
    def save_best_network(self, epoch, iter_step, optimizer):
