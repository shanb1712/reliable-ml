...
def json_to_dict(args):
    items = {}
    for key, value in args.items():
        # new_key = f"{parent_key}.{key}" if paren/t_key else key
        if isinstance(value, dict):
            items[key] = json_to_dict(value)
        else:
            items[key] = value
    return items
...
def parse(args):
    ...
    ''' set log directory '''
    experiments_root = os.path.join(opt['path']['base_dir'], '{}_{}_{}'.format(opt['name'], get_timestamp(), shortuuid.ShortUUID().random(length=4)))
    ...
    if args.distributed_worker_id is not None:
        opt['distributed_worker_id'] = args.distributed_worker_id
        opt['datasets'][opt['phase']]['which_dataset']['args']['skip_n_samples'] = opt['datasets'][opt['phase']]['which_dataset']['args']['data_len'] * args.distributed_worker_id

    if args.finetune_loss is not None:
        opt['train']['finetune_loss'] = args.finetune_loss

    if args.prediction_time_step is not None:
        opt['train']['prediction_time_step'] = args.prediction_time_step

    if args.wandb_run_name is not None:
        opt['wandb']['wandb_run_name'] = args.wandb_run_name
