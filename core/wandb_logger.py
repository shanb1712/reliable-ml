...
class WandbLogger:
    ...
    def __init__(self, opt):
        ...
        if self._wandb.run is None:
            if opt["wandb"]["wandb_run_name"]:
                self._wandb.init(project=opt['wandb']['project'], name=opt["wandb"]["wandb_run_name"],config=opt, dir='./experiments')
            else:
        ...
        if self.config.get('log_eval', None):
            self.eval_table = self._wandb.Table(columns=['fake_image', 'sr_image', 'hr_image', 'psnr', 'ssim'])
        else:
    ...
    def log_image(self, key_name, image_array, caption=None, commit=True):
        """
        ...
        """
        if caption is None:
            self._wandb.log({key_name: self._wandb.Image(image_array)}, commit=commit)
        else:
    ...
    def log_images(self, key_name, list_images, commit=True):
        ...
        """
        self._wandb.log({key_name: [self._wandb.Image(img) for img in list_images]}, commit=commit)

    def log_bar_chart(self, key_name, barcahrt_table, label, value, commit=True):
        """
        Log list of image array onto W&B
        ...
        list_images: list of numpy image arrays
        """
        self._wandb.log({key_name: self._wandb.plot.bar(barcahrt_table,label,value)}, commit=commit)
    ...
    def log_eval_data(self, fake_img, sr_img, hr_img, psnr=None, ssim=None):
        ...
        if psnr is not None and ssim is not None:
            self.eval_table.add_data(self._wandb.Image(fake_img), self._wandb.Image(sr_img), self._wandb.Image(hr_img), psnr, ssim)
        else:
            self.infer_table.add_data(self._wandb.Image(fake_img), self._wandb.Image(sr_img), self._wandb.Image(hr_img))
