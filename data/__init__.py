...
def define_dataset(logger, opt):
    ...
    phase_dataset = init_obj(dataset_opt, logger,
                             default_file_name=opt['datasets'][opt['phase']]['dataloader']['default_file_name'],
                             init_type='Dataset')
    val_dataset = None
...
def define_dataset_for_phase(logger, opt, phase):
    dataset_opt = opt['datasets'][phase]['which_dataset']
    phase_dataset = init_obj(dataset_opt, logger, default_file_name='data.bounds_finetune_dataset', init_type="BoundsInpaintDataset")
    return phase_dataset


def define_dataloader_for_phase(logger, opt, phase):
    dataloader_args = opt['datasets'][phase]['dataloader']['args']
    worker_init_fn = partial(Util.set_seed, gl_seed=opt['seed'])
    phase_dataset = define_dataset_for_phase(logger, opt, phase)
