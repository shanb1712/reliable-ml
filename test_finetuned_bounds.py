...
def run_test(diffusion_with_bounds, wandb_logger, device, test_step, test_loader):
    with torch.no_grad():
        ...
        for test_idx, test_data in enumerate(test_loader):
            ...
            noise_level = (opt["var"] / 2) * (torch.rand_like(test_gt_image)* 2 - 1)
            test_masked_image = test_gt_image + noise_level
            test_masked_image = torch.clamp(test_masked_image, -1.0, 1.0)

            test_pred_lower_bound, test_pred_upper_bound = diffusion_with_bounds(test_masked_image)
            test_pred_lower_bound = test_pred_lower_bound.unsqueeze(1)
            test_pred_upper_bound = test_pred_upper_bound.unsqueeze(1)

            vis_util.log_train(diffusion_with_bounds, wandb_logger, test_pred_lower_bound, test_pred_upper_bound, test_masked_image, test_gt_image)
        ...
        pred_test_calibrated_l = (all_test_pred_lower_bounds / diffusion_with_bounds.pred_lambda_hat).clamp(-1., 1.)
        pred_test_calibrated_u = (all_test_pred_upper_bounds * diffusion_with_bounds.pred_lambda_hat).clamp(-1., 1.)
        # endregion
